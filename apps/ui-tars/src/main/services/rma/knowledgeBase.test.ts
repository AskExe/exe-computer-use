...
describe('KnowledgeBase', () => {
  ...
  it('stores and retrieves a fact', () => {
    kb.addStructuredFact({ text: 'use Ctrl+S to save on github.com', domain: 'github.com' });
    const facts = kb.getRelevantFacts('open github.com and save a file');
    expect(facts.map((f) => f.text)).toContain('use Ctrl+S to save on github.com');
  });

  it('returns global facts for any instruction', () => {
    kb.addStructuredFact({ text: 'always wait 2 seconds after clicking submit', domain: null });
    const facts = kb.getRelevantFacts('do anything');
    expect(facts.map((f) => f.text)).toContain('always wait 2 seconds after clicking submit');
  });
  ...
  it('limits facts to MAX_FACTS_PER_RUN', () => {
    for (let i = 0; i < 20; i++) kb.addStructuredFact({ text: `fact ${i}`, domain: null });
  ...
  it('formats facts as system prompt injection string', () => {
    kb.addStructuredFact({ text: 'click Accept cookies first', domain: 'example.com' });
